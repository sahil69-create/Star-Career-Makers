...
if(contactForm){contactForm.addEventListener('submit',async e=>{e.preventDefault();const name=document.getElementById('name').value.trim();const phone=document.getElementById('phone').value.trim();const email=document.getElementById('email').value.trim();const message=document.getElementById('message').value.trim();const fileInput=document.getElementById('resume');const file=fileInput&&fileInput.files&&fileInput.files[0]?fileInput.files[0]:null;if(!name||!phone||!email||!message){alert('Please fill all fields');return}let resumeName='';let resumeType='';let resumeBase64='';if(file){resumeName=file.name;resumeType=file.type||'application/octet-stream';resumeBase64=await toBase64(file)}const fd=new FormData();fd.append('name',name);fd.append('phone',phone);fd.append('email',email);fd.append('message',message);if(resumeBase64){fd.append('resumeName',resumeName);fd.append('resumeType',resumeType);fd.append('resumeBase64',resumeBase64)}try{const useNoCors=window.location.protocol==='file:';const opts=useNoCors?{method:'POST',body:fd,mode:'no-cors'}:{method:'POST',body:fd};const res=await fetch(APP_SCRIPT_URL,opts);if(useNoCors||res.ok){alert('Thank you! Your message has been recorded.');contactForm.reset()}else{alert('Submission failed. Please try again.')}}catch(err){alert('Network error. Please try again later.')}})}
