...
if(contactForm){contactForm.addEventListener('submit',async e=>{e.preventDefault();const name=document.getElementById('name').value.trim();const phone=document.getElementById('phone').value.trim();const email=document.getElementById('email').value.trim();const message=document.getElementById('message').value.trim();if(!name||!phone||!email||!message){alert('Please fill all fields');return}const body=new URLSearchParams({name,phone,email,message});try{const useNoCors=window.location.protocol==='file:';const opts=useNoCors?{method:'POST',body,mode:'no-cors'}:{method:'POST',body};const res=await fetch(APP_SCRIPT_URL,opts);if(useNoCors||res.ok){alert('Thank you! Your message has been recorded.');contactForm.reset()}else{alert('Submission failed. Please try again.')}}catch(err){alert('Network error. Please try again later.')}})}
